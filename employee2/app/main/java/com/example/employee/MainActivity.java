...
// TODO Auto-generated method stub Toast.makeText(this, "buttonclicked", 15000).show();
        if(v.equals(btnsubmit))
        {
            String sid=txtid.getText().toString();
            String sname=txtname.getText().toString(); String sage=txtage.getText().toString();
            String saddress=txtaddress.getText().toString();
            MyDatabase dat=new MyDatabase(this,MyDatabase.DATABASE_NAME, null,1);
            SQLiteDatabase database=dat.getWritableDatabase(); ContentValues cv= new ContentValues();
            cv.put("id", sid);
            cv.put("name", sname);
            cv.put("age",sage ); cv.put("address", saddress);
